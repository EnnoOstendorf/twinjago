...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/':
            existing_data = {}
            if os.path.exists('mqtt_moving_dummy_json_data.txt'):
                with open('mqtt_moving_dummy_json_data.txt', 'r', encoding='utf-8') as f:
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/stop":
                ...
                if os.path.exists('moving-PIDs.txt'):
                    with open('moving-PIDs.txt', 'r') as f:
                        lines = f.readlines()
                    with open('moving-PIDs.txt', 'w') as f:
                        for line in lines:
                ...
                try:
                    panel_id = f"panel{data.get('panelId')}"
                    if not os.path.exists('mqtt_moving_dummy_json_data.txt'):
                        self.send_response(404)
                        self.send_header('Content-Type', 'application/json')
                        self.send_header('Access-Control-Allow-Origin', '*')
                        self.end_headers()
                        self.wfile.write(json.dumps({"error": "mqtt_moving_dummy_json_data.txt not found"}).encode('utf-8'))
                        return
               ...
               # Panels in eine aufsteigende Reihenfolge bringen
                        ...
                        with open('mqtt_moving_dummy_json_data.txt', 'w', encoding='utf-8') as f:
                ...
                try:
                    ...
                    new_panel_data = {
                        ...
                        "mqtt_meta": {
                            "type": 1,
                            "name": f"new_moving_object_{next_panel_id}",
                            "desc": "simulation",
                            "payloadType": "json",
                            "payloadStructure": [
                                {
                    ...
                    print("Fehler: Die Datei 'mqtt_moving_dummy_json_data.txt' wurde nicht gefunden.")
                except json.JSONDecodeError:
                    print("Fehler: Die Datei 'mqtt_moving_dummy_json_data.txt' enthält ungültiges JSON.")
                except Exception as e:
            ...
            # Starten des Scripts 
            process = subprocess.Popen(['python3', 'mqtt_moving_dummy_publisher_06.py', device_id])
            pid = process.pid
            # Schreiben der PID-Daten
            with open('moving-PIDs.txt', 'a') as f:
...
def run(server_class=HTTPServer, handler_class=SimpleHTTPRequestHandler, host='freetwin.de', port=3557):
