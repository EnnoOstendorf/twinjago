...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def check_list_mqtt_status(self, new_id):
        print("checklist")
        global device_ids  # Globale Referenz auf die Liste der Device-IDs
        return "TRUE" if new_id in device_ids else "FALSE"
            ...
            if self.path == "/stopdupli":
                ...
                if mqtt_status_orig == "TRUE":
                    ...
                    with open('Double-PIDs.txt', 'a') as f:
                        f.write(f"{dupliPanelId},{new_id},{double_pid}\n")
                else:
                    self.send_response(200)
                    self.send_header('Content-Type', 'application/json')
                    self.send_header('Access-Control-Allow-Origin', '*')
...
def run(server_class=HTTPServer, handler_class=SimpleHTTPRequestHandler, host='freetwin.de', port=3556):
